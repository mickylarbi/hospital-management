...
export class UserService {
  ...
  async findOne(id: string) {
    return await this.prismaService.user.findUnique({ where: { id }, include: { patients: true, doctors: true } });
  }
  ...
  update(id: string, note: UpdateUserDto) {
  ...
  async remove(id: number) {
    ...
    const allWithHashedPasswords = [
      ...patients.map(e => {
        return { ...e, password: bcrypt.hashSync(Buffer.from(e.password, 'base64').toString('utf-8'), 10) }
      }),
      ...doctors.map(e => {
        return { ...e, password: bcrypt.hashSync(Buffer.from(e.password, 'base64').toString('utf-8'), 10) }
      })
    ...
    await this.prismaService.user.createMany({ data: allWithHashedPasswords })
